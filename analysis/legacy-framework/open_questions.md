...
**Fix (eval harness):** ✅ **dedup landed (2026-06-29)** — `cross_val_accuracy(dedup=True)` now collapses identical (x, y, ctx) constraints before folding, so `value_model.py eval` reports the leakage-free number (and `_eval_dedup.py` is the fast single-split). ⏸ **Vectorizing `train()` is DEFERRED until after Phase 3 grading:** changing the trainer's numerics would alter the model and jeopardize the *reproducibility* of the already-registered Phase 2 predictions (`predictions/model-verdicts-msh.md`), and adding numpy would break this module's `dependencies = []` design. Until then full dedup CV is correct but slow at 865 features — use `_eval_dedup.py`. Logged 2026-06-26, updated 2026-06-29.
