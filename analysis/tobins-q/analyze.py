...
import pandas as pd
import numpy as np
from gatheringdata.charts import ACCENT, ACCENT_DARK, TEXT, TEXT_SECONDARY, BORDER, apply_blog_style, save
...
_DEFAULT_DATA_DIR    = _REPO_ROOT / "public" / "data"
_CACHE_DIR = Path(__file__).parent / ".cache"

OUTPUT_DIR: Path = _DEFAULT_OUTPUT_DIR  # overridden by --output-dir at runtime
DATA_DIR:   Path = _DEFAULT_DATA_DIR

# Local color aliases for chart-specific roles
_HIGHLIGHT = ACCENT_DARK   # official Q dots / secondary series
_WARN      = TEXT_SECONDARY  # annotations, footnotes
...
def chart_official_q(q_official: pd.Series) -> None:
    ...
    ax.fill_between(q_official.index, q_official, geo_mean, where=(q_official >= geo_mean),
                    alpha=0.15, color=ACCENT_DARK, label="_nolegend_")
    ax.fill_between(q_official.index, q_official, geo_mean, where=(q_official < geo_mean),
                    alpha=0.10, color=ACCENT, label="_nolegend_")
    ax.plot(q_official.index, q_official, color=ACCENT_DARK, linewidth=1.8, label="Official Q (Z.1)")
    ax.axhline(geo_mean, color=TEXT_SECONDARY, linestyle="--", linewidth=1.2,
               label=f"Geometric mean ({geo_mean:.2f})")
    ax.axhline(1.0, color=BORDER, linestyle=":", linewidth=1.0, label="Q = 1.0")
    ...
    ax.annotate(
        ...
        fontsize=9,
        color=ACCENT_DARK,
        arrowprops=dict(arrowstyle="->", color=ACCENT_DARK, lw=1.2),
    )

    apply_blog_style(ax,
                     title="Tobin's Q — U.S. Nonfinancial Corporations, 1945–present",
                     ylabel="Q ratio (market value / replacement cost)")
    ax.legend(fontsize=9, frameon=False)
    _save(fig, "q_official.png")
...
def chart_decomposition(decomp: pd.DataFrame) -> None:
    ...
    colors = [ACCENT_DARK, ACCENT, "#9ab8c8", "#c8dce6"]
    ...
    apply_blog_style(ax,
                     title="Gross Asset Composition: Nonfinancial Corporations, 1945–present",
                     ylabel="Share of measured gross assets")
    _save(fig, "q_decomposition.png")
...
def chart_proxy_vs_official(proxy_df: pd.DataFrame, market_index_source: str) -> None:
    ...
    ax.plot(proxy.index, proxy, color=ACCENT_DARK, linewidth=1.5,
            label="Monthly proxy (this analysis)", alpha=0.9)
    ax.scatter(official.index, official, color=ACCENT, s=18, zorder=5,
    ...
    ax.annotate(note, xy=(0.01, 0.02), xycoords="axes fraction", fontsize=7.5, color=TEXT_SECONDARY)
    apply_blog_style(ax, title="Monthly Q Proxy vs. Official Quarterly Q", ylabel="Q ratio")
    _save(fig, "q_proxy_vs_official.png")
...
def chart_validation_error(val: pd.DataFrame) -> None:
    ...
    ax = axes[0]
    ax.bar(val.index, val["pct_error"], color=ACCENT_DARK, alpha=0.7, width=60)
    ax.axhline(0, color=TEXT_SECONDARY, linewidth=1)
    ax.annotate(f"MAE: {mae:.3f}  |  Mean abs % error: {mpct:.1f}%",
                xy=(0.02, 0.96), xycoords="axes fraction", fontsize=9, color=TEXT_SECONDARY,
                verticalalignment="top")
    apply_blog_style(ax, title="Proxy Error Over Time (% of official Q)", ylabel="Error (%)")

    direction_accuracy = val["direction_match"].mean() * 100
    ax2 = axes[1]
    ax2.hist(val["pct_error"], bins=30, color=ACCENT_DARK, alpha=0.75, edgecolor="white")
    ax2.axvline(0, color=ACCENT, linewidth=1.5, linestyle="--")
    ax2.annotate(f"Direction accuracy: {direction_accuracy:.0f}%",
                 xy=(0.97, 0.96), xycoords="axes fraction", fontsize=9, color=TEXT_SECONDARY,
                 horizontalalignment="right", verticalalignment="top")
    apply_blog_style(ax2, title="Error Distribution",
                     xlabel="Proxy − Official (% of official)", ylabel="Count (quarters)")

    fig.tight_layout()
    _save(fig, "q_proxy_error.png")
...
def chart_current_estimate(proxy_df: pd.DataFrame, val: pd.DataFrame) -> None:
    ...
    ax.plot(proxy.index, proxy, color=ACCENT_DARK, linewidth=1.8, label="Monthly proxy")
    ax.scatter(official.index, official, color=ACCENT, s=18, zorder=5,
               label="Official Q (Z.1)", linewidths=0)

    geo_mean = np.exp(np.log(official[official > 0]).mean())
    ax.axhline(geo_mean, color=TEXT_SECONDARY, linestyle="--", linewidth=1.1,
               label=f"Geometric mean ({geo_mean:.2f})")
    ...
    ax.annotate("Shaded region: most recent quarter, proxy only (no official Z.1 data yet)",
                xy=(0.01, 0.02), xycoords="axes fraction", fontsize=7.5, color=TEXT_SECONDARY)

    ax.legend(fontsize=9, frameon=False)
    apply_blog_style(ax, title="Tobin's Q — Official (Quarterly) and Monthly Proxy",
                     ylabel="Q ratio")
    _save(fig, "q_current_estimate.png")
...
def main() -> None:
    ...
    # --- Export data ---
    csv_path = DATA_DIR / "q_monthly_proxy.csv"
    proxy_df[["Q_official", "Q_proxy"]].round(4).to_csv(csv_path)
    print(f"\n  [data] exported {csv_path}")
