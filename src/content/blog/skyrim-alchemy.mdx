...
Both are genuine geographic concentrations, not artifacts — the LP has access to all possible ingredient pairs and still routes decisively.
...
The Pure Mage allocates 12.5 hours to The Reach and 7.5 to Winterhold. This is counterintuitive — Whiterun is where Jazbay Grapes grow (canonical Fortify Magicka ingredient), and the frozen tundra makes more intuitive sense for a caster build. But the LP found Grass Pod and Wild Grass Pod in The Reach, which share the Restore Magicka effect and are abundant enough to outcompete the Jazbay-based route. The canonical pair selection was hiding this — it had locked in a Whiterun-anchored recipe and never considered The Reach alternative.
...
The slope of each curve is the shadow price of the time constraint: how much additional weighted objective you gain per additional foraging hour. All three builds show roughly linear returns up to ~20 hours, then the curves flatten as the model runs into ingredient scarcity that more time alone can't fix. At that point, you're not time-constrained; you're ingredient-constrained, and the only fix is a better route, not more hours.
...
Everything above assumes you're brewing with two ingredients per potion. But Skyrim's alchemy bench actually accepts up to three — and the interaction rule is interesting: an effect activates if it appears on **at least two** of the three ingredients you combine. A third ingredient can unlock an entirely new effect without replacing either of the first two.

There's a catch, though. Mixing positive and negative effects in the same brew is a trap. A potion's type — beneficial or harmful — is determined by whichever effect has the highest base gold value. If you accidentally brew a negative effect into a potion, it applies to **you** when you drink it. If a positive effect sneaks into a poison, it applies to your **enemy** when they're hit. Either way, you've buffed the wrong side. The analysis only considers "pure" three-ingredient combinations where every activated effect shares the same polarity.

With that filter in place, this becomes a secondary optimization problem. The primary LP already told you how many total units of each ingredient you'll collect from foraging. A second LP then asks: given that ingredient stockpile, what's the optimal mix of two-ingredient and three-ingredient brews?
...
The single most valuable three-ingredient combo on a per-batch basis remains the Pure Mage's Magicka triple — Briar Heart + Jazbay Grapes + Moon Sugar or Fire Salts, activating Fortify Magicka + Regenerate Magicka + Restore Magicka simultaneously. The synergy score is 9 versus a best-pair score of 3. But at only +2% overall, the constraint isn't the brew mechanic — it's that the foraging LP collects limited quantities of the required ingredients and can't easily increase them without redirecting hours away from other priorities.
